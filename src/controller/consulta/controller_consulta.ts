import {
  ...
  ERROR_NOT_DELETED,
  ERROR_NOT_FOUND,
  ERROR_NOT_FOUND_APPOINTMENTS,
  ERROR_NOT_FOUND_CLIENT,
  ERROR_NOT_FOUND_PROFESSIONAL,
...
import {
  ...
  deleteAppointment,
  selectAppointment,
  selectAppointmentByProfessional,
  selectAppointmentByUserId,
  updateAppointment,
  verificarConsultaCliente,
  verificarConsultaExistente,
} from "../../model/DAO/consulta/consulta";
import { buscarPsicologo } from "../../model/DAO/psicologo/usuario";

export async function setCadastrarConsulta(
  ...
  try {
    ...
    const existingAppointment = await verificarConsultaExistente(
      idProfessional,
      newData
    );
    if (existingAppointment) {
      return ERROR_CONFLICT_APPOINTMENT; 
    }

    const existingClientAppointment = await verificarConsultaCliente(
      idClient,
      newData
    );
    ...
    let newAppointment = await createAppointment(
      idProfessional,
      idClient,
      newData
    );
...
export async function getBuscarConsultasPorProfissional(id: number) {
  try {
    ...
    let getAppointmentByProfessional = await selectAppointmentByProfessional(
      id
    );
...
export async function setAtualizarConsulta(
  id: number,
  data: Date,
  contentType: string | undefined
) {
...
export async function getAllAppointmentByUserId(user_id: number) {
  try {
    if (!user_id || !isValidId(user_id)) {
      return ERROR_REQUIRED_FIELDS;
    }

    let searchAppointment = await selectAppointmentByUserId(user_id);
