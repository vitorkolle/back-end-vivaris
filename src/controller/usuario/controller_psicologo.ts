...
import { ERROR_AGE_NOT_VALID, ERROR_ALREADY_EXISTS_ACCOUNT_CIP, ERROR_ALREADY_EXISTS_ACCOUNT_CPF, ERROR_ALREADY_EXISTS_ACCOUNT_EMAIL, ERROR_CONTENT_TYPE, ERROR_DATE_NOT_VALID, ERROR_INTERNAL_SERVER, ERROR_INTERNAL_SERVER_DB, ERROR_NOT_CREATED, ERROR_NOT_FOUND, ERROR_NOT_UPDATED, ERROR_REQUIRED_FIELDS, SUCCESS_CREATED_ITEM } from "../../../module/config";
...
import { atualizarPsicologo, buscarPsicologo, criarNovoPsicologo, listarPsicologos, logarPsicologo } from "../../model/DAO/psicologo/usuario";
import { getBuscarSexo } from "./controller_usuario";
...
export async function setAtualizarPsicologo(user: TProfessional, contentType: string | undefined, id: number) {
    try {
        if (String(contentType).toLowerCase() !== 'application/json') {
            return ERROR_CONTENT_TYPE
        }
        ...
        if (
            ...
            !user.cip || user.cip.length !== 9 ||
            !user.data_nascimento || !validarData(user.data_nascimento.toString()) || validarIdade(user.data_nascimento) < 18 || 
            !user.email || !isValidEmail(user.email)  ||
            !user.senha || !isValidPassword(user.senha)  ||
            !user.telefone || user.telefone.length !== 11 || typeof user.telefone !== 'string' ||
            !user.id_sexo || !isValidId(user.id_sexo) || !getBuscarSexo(user.id_sexo)
        ){
            if(!validarData(user.data_nascimento.toString())){
                return ERROR_DATE_NOT_VALID
            }
        ...
        let validatePsicologo = await buscarPsicologo(id)

        if (!validatePsicologo) {
            return ERROR_NOT_FOUND
        }

        const updatedPsicologo = await atualizarPsicologo(id, inputData)
        ...
        console.error('Erro ao tentar atualizar um psicólogo:', error);
        return ERROR_INTERNAL_SERVER
    }
}
